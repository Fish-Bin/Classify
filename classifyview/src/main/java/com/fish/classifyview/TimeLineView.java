...
public class TimeLineView extends View{
    ...
    public TimeLineView(Context context, @Nullable AttributeSet attrs) {
        ...
        if(typedArray!=null){
            ...
            typedArray.recycle();
            LogBin.i("mMarkerSize="+mMarkerSize+"   mLinSize="+mLineSize);
        }
        if(mBeginLine!=null){
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {//此时拿到的是控件的宽度和高度
        ...
        LogBin.i("w="+w+"  h="+h+"   oldw="+oldh+"   oldh="+oldh);
        initDrawableSize();
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {//拿到的是控件的位置
        ...
        LogBin.i("left="+left+"  top="+top+"   right="+right+"  bottom="+bottom);
        LogBin.i("width="+getWidth()+" height="+getHeight());
    }
